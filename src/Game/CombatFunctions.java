...
public class CombatFunctions {
	 ...
	 * @param mapState: The map layer that contains objects that should block projectiles
	 * @param gjk: must pass the GJK to handle collisions
	 * @param bullet: the bullet that you are checking for collisions
	 */
	public static void bulletHitHandler( EngineState mainState , EngineState mapState , GJK gjk , int bullet ) {
		final PhysicsPCollisionBody bulletBody = 
				mainState.getComponentAt( PhysicsPCollisionBody.class , bullet );
		...
		for (int i = mainState.getInitialSetIndex(MobSet.class);
			...
			final PhysicsPCollisionBody mobBody =
					mainState.getComponentAt(
						PhysicsPCollisionBody.class, i);
			
			// if collision detected
			if (Systems.arePCollisionBodiesColliding(gjk, bulletBody, mobBody)) {
				removeBullet(mainState , bullet);
				mainState.getComponentAt(HitPoints.class, i).hurt(GameConfig.BULLET_DAMAGE);
				if ( mainState.getComponentAt(HitPoints.class, i).getHP() <= 0 ) 
		...
		for (PhysicsPCollisionBody wall : mapState.getRawComponentArrayListPackedData(
				PhysicsPCollisionBody.class)) {
			if ( Systems.arePCollisionBodiesColliding(gjk, bulletBody , wall )) {
				removeBullet( mainState , bullet );
				break;
			}
		}
	 ...
	 */
	public static void handleMobHitPlayer( EngineState engineState , GJK gjk , int mob , int player ) {
		final PhysicsPCollisionBody playerBody = 
				engineState.getComponentAt(PhysicsPCollisionBody.class, player);
		final PhysicsPCollisionBody mobBody = 
