...
/**
 * Map godly class for storing, adding, and getting the map layers
 * Date: April 12, 2019
...
public class Map
{
	// store the layers of the map, where each layer is an ECS
	...
	{
		try {
			// parse the json file for the map config, generated by
			// the application Tiled
			...
			while (configReader.hasNextLine()) {
				...
				if (tempString.length > 1
					...
					// stores the height and width of the
					// map
		...
		try {
			...
			while (mapReader.hasNextLine()) {
				...
				if (tempList.length > 1) {
					// parse the column, tile count, and
					// other userful information for the map
					switch (tempList[1]) {
					case "columns":
	...
	/**
	 * Add the collision body for a tile at an index at collision layer
	 * (layer 0)
	 *
	 * @param  row	the row number of the tile
	 * @param  col	the column number of the tile
	 * @param  tileCord	the coordinate of the tile inside the tile set
	 * @param  nextFreeIndex	the next free index inside ECS
	 *  @return      void
	 */
	 ...
	 *
	 * @param  tileCord	the coordinate of the tile inside the tile set
	 *  @return     Optional <Render> checks if it is a special tile, and if
	 * it is it will return an image that corresponds to the building
	 * (description of special tile cann be foud aroudn line 266 )
	 */
	...
	{
		// fill up the wallstate with all booleans that equals to false
		wallState = new ArrayList<Boolean>(
			Collections.nCopies(tileCount, false));

		Vector2f cbwc = new Vector2f(0f, 0f);
		// setting up the collision body for each coordinate in tile set
